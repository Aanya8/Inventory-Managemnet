...
export default {
  ...
  setup() {
    ...
    // Currency symbol derived from current currency setting
    ...
    // Max revenue used for bar chart scaling — computed once, not per call
    ...
    const formatMonth = (monthStr) => {
      // Parse YYYY-MM string and format using locale-aware date formatting
    ...
    const getChangeValue = (current, previous) => {
      ...
      if (change > 0) return `+${sym}${Math.abs(change).toLocaleString()}`
      if (change < 0) return `-${sym}${Math.abs(change).toLocaleString()}`
      return `${sym}0`
    }
