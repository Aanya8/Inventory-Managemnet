...
def get_restocking_recommendations(budget: float = 10000):
    """Return inventory items below reorder point, ranked by deficit, fitting within budget."""
    below_reorder = [
        item for item in inventory_items
        if item['quantity_on_hand'] <= item['reorder_point']
    ]
    # Most critical items first (largest deficit)
    below_reorder.sort(
        key=lambda x: x['reorder_point'] - x['quantity_on_hand'],
        reverse=True
    )
    ...
    for item in below_reorder:
        # Order enough units to bring stock back up to the reorder point level
        qty_to_order = item['reorder_point'] - item['quantity_on_hand']
        cost = round(qty_to_order * item['unit_cost'], 2)
...
def submit_restocking_order(request: SubmitRestockingOrderRequest):
    """Submit a restocking order. Adds it to the orders list with a 7-day delivery lead time."""
    ...
    # Use "Multiple" for warehouse/category when items span several
    warehouses = list({item.warehouse for item in request.items})
    categories = list({item.category for item in request.items})
    warehouse = warehouses[0] if len(warehouses) == 1 else "Multiple"
    ...
    order = {
        "id": order_id,
        "order_number": f"RST-{order_id[:6].upper()}",
    ...
    # Append to in-memory orders so GET /api/orders includes it
    orders.append(order)
